...
image:test1/t09.png[geometry,50%]

Here is a comparison under paraview between the exact solution and the calculated solution, at different time :

image:test1/t01.png[geometry,50%]

image:test1/t05.png[geometry,50%]

image:test1/t09.png[geometry,50%]

We conclude that at every time, the stem:[L^2] error slope is close to 2, which is what we expect to have,
and the stem:[H^1] error slope is also close to 1. The différence can be explain by the fact we took only 3 hsize, and the result could be better with lower hsize (but the running time can become very long).
