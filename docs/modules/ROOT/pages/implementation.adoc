...
Remember that stem:[A \in H_{A_D}^{curl}], but here, to siplify, we will concider that stem:[A \in H^{1}], so we have to take a correct function space for stem:[A^n] :
[source,cpp]
----
auto Ah = Pchv<1>(mesh);
----
...
[source,cfg]
----
include::https://raw.githubusercontent.com/feelpp/mqs/master/src/cases/test2/test22.cfg
----
...
Reason is, when we solve (and export) the system with :
...
we dont have good results. For exemple, at the time stem:[t=dt], the solution we obtain is not close to the exact one, and the error bad.
...
[source,cpp]
----
include::https://raw.githubusercontent.com/feelpp/mqs/master/src/mqs-blockform.cpp
----